...
7. All LD context brokers come with the NGSI-LD core context that holds all the NGSI-LD base information to make the context brokers work. You can add your own context files to your setup by adding them to the folder [context-files](../general/context-files/). A sample context file for mapping context information from the Brick Schema to our data models is already provided and loaded in the IoT Agent. Upon creation of context information to orion, you have to provide a context file, as referred to in the [LD tutorials](https://github.com/FIWARE/tutorials.NGSI-LD). The provided links to the context file need to be accessible for the context broker. Therefore, in our case, you can use "http://context-server:80/context.jsonld" for creating entities, refer to your own context files located somewhere else or refer to the [smart data models from FIWARE](https://smartdatamodels.org/). The context server is just a way to make custom context files available. If you refer to publicly available files, the context server is not needed. Nevertheless, in our opinion, it makes sense to store all context files in one location.

   **Note:** Orion LD caches context files! In case you change a context file on the fly, make sure to delete the cached file first! All cached context files for the Orion LD can be seen at its endpoint `/ngsi-ld/v1/jsonldContexts/`. If already cached by Orion LD, the custom context file can be deleted via a DELETE request to Orion LD's endpiont `/ngsi-ld/v1/jsonldContexts/http://context-server:80/context.jsonld`
